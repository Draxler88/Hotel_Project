...
$capa = $connexion->prepare("SELECT * FROM capacité_chambre WHERE Id_capacité=?");
$capa->execute([$_GET["idModCapacite"]]);
$result = $capa->fetchObject();
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <div class="container">
        ...
        <form action="<?=$_SERVER["PHP_SELF"]?>" method="POST">
            <div class="mb-3">
                ...
                <input type="text" name="titre_capacite" id="titre" class="form-control" value="<?=$result->Titre_capacite?>">
            </div>
            <div class="mb-3">
                <label for="numero" class="form-label">Numéro Capacité</label>
                <input type="number" name="numero_capacite" id="numero" class="form-control" value="<?=$result->Numero_capacite?>">
            </div>
